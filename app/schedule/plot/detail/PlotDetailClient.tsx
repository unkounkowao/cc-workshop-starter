...
export default function PlotDetailClient() {
  ...
  if (notFound || !entry) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-[#21ecff]-50 flex items-center justify-center p-8">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-[#21ecff]-50">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-3">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {entry.plotRole && (
                <span className="text-xs font-medium px-2.5 py-1 rounded-full bg-[#21ecff]-50 text-[#21ecff]-600 border border-[#21ecff]-200">
                  {entry.plotRole}
                </span>
              )}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-4">
            ...
            {(entry.cause || entry.result || entry.foreshadowing || entry.payoff) && (
              <div className="bg-[#21ecff]-50 rounded-xl shadow-sm border border-[#21ecff]-100 p-5">
                <h2 className="text-xs font-semibold text-[#21ecff]-700 uppercase tracking-wide mb-4">出来事の詳細</h2>
        ...
        <nav className="flex items-center justify-between gap-4 pt-2" aria-label="前後のエントリへの移動">
          {prevEntry ? (
            <Link
              ...
              className="flex-1 flex flex-col items-start gap-0.5 px-4 py-3 bg-white rounded-xl shadow-sm border border-slate-100 hover:border-[#21ecff]-200 hover:shadow-md transition-all group max-w-xs"
            >
              <span className="text-xs text-slate-400 group-hover:text-[#21ecff]-500 transition-colors">← 前へ</span>
              <span className="text-sm font-medium text-slate-700 group-hover:text-[#21ecff]-700 transition-colors line-clamp-1">{prevEntry.title}</span>
            </Link>
          ) : <div className="flex-1 max-w-xs" />}
          {nextEntry ? (
            <Link
              href={`/schedule/plot/detail?id=${nextEntry.id}`}
              className="flex-1 flex flex-col items-end gap-0.5 px-4 py-3 bg-white rounded-xl shadow-sm border border-slate-100 hover:border-[#21ecff]-200 hover:shadow-md transition-all group max-w-xs"
            >
              <span className="text-xs text-slate-400 group-hover:text-[#21ecff]-500 transition-colors">次へ →</span>
              <span className="text-sm font-medium text-slate-700 group-hover:text-[#21ecff]-700 transition-colors line-clamp-1">{nextEntry.title}</span>
            </Link>
          ) : <div className="flex-1 max-w-xs" />}
        </nav>
      </div>
