...
export default function PlotDetailClient() {
  ...
  if (notFound || !entry) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-[#1fefec]/10 flex items-center justify-center p-8">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-[#1fefec]/10">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-3">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {entry.plotRole && (
                <span className="text-xs font-medium px-2.5 py-1 rounded-full bg-[#1fefec]/10 text-[#1fefec] border border-[#1fefec]/30">
                  {entry.plotRole}
                </span>
              )}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-4">
            ...
            {(entry.cause || entry.result || entry.foreshadowing || entry.payoff) && (
              <div className="bg-[#1fefec]/10 rounded-xl shadow-sm border border-[#1fefec]/20 p-5">
                <h2 className="text-xs font-semibold text-[#1fefec] uppercase tracking-wide mb-4">出来事の詳細</h2>
        ...
        <nav className="flex items-center justify-between gap-4 pt-2" aria-label="前後のエントリへの移動">
          {prevEntry ? (
            ...
            >
              ...
              <span className="text-sm font-medium text-slate-700 group-hover:text-[#1fefec] transition-colors line-clamp-1">{prevEntry.title}</span>
            </Link>
          ...
          {nextEntry ? (
            ...
            >
              ...
              <span className="text-sm font-medium text-slate-700 group-hover:text-[#1fefec] transition-colors line-clamp-1">{nextEntry.title}</span>
            </Link>
          ) : <div className="flex-1 max-w-xs" />}
        </nav>
      </div>
