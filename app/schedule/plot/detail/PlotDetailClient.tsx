...
export default function PlotDetailClient() {
  ...
  useEffect(() => {
    ...
    if (!e || e.type !== 'plot') { setNotFound(true); return }
    setEntry(e)
  ...
  if (notFound || !entry) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-[#1fefec]-50 flex items-center justify-center p-8">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-[#1fefec]-50">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-3">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {entry.plotRole && (
                <span className="text-xs font-medium px-2.5 py-1 rounded-full bg-[#1fefec]-50 text-[#1fefec]-600 border border-[#1fefec]-200">
                  {entry.plotRole}
                </span>
              )}
            </div>
            <div className="flex items-center gap-2">
              <Link
                href={`/schedule/plot/edit?id=${entry.id}`}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-4">
            ...
            {(entry.cause || entry.result || entry.foreshadowing || entry.payoff) && (
              <div className="bg-[#1fefec]-50 rounded-xl shadow-sm border border-[#1fefec]-100 p-5">
                <h2 className="text-xs font-semibold text-[#1fefec]-700 uppercase tracking-wide mb-4">出来事の詳細</h2>
                <dl className="space-y-4">
                  {entry.cause && <DetailField label="原因・きっかけ" value={entry.cause} />}
                  {entry.result && <DetailField label="結果・影響" value={entry.result} />}
                  {entry.foreshadowing && <DetailField label="伏線" value={entry.foreshadowing} />}
                  {entry.payoff && <DetailField label="伏線回収" value={entry.payoff} />}
                </dl>
              </div>
            )}
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-5">
              <dl className="space-y-4">
                ...
                {entry.plotRole && (
                  <div>
                    <dt className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">プロットロール</dt>
                    <dd className="text-sm text-slate-800">{entry.plotRole}</dd>
                  </div>
                )}
              </dl>
            </div>
        ...
        <nav className="flex items-center justify-between gap-4 pt-2" aria-label="前後のエントリへの移動">
          {prevEntry ? (
            <Link
              ...
              className="flex-1 flex flex-col items-start gap-0.5 px-4 py-3 bg-white rounded-xl shadow-sm border border-slate-100 hover:border-[#1fefec]-200 hover:shadow-md transition-all group max-w-xs"
            >
              <span className="text-xs text-slate-400 group-hover:text-[#1fefec]-500 transition-colors">← 前へ</span>
              <span className="text-sm font-medium text-slate-700 group-hover:text-[#1fefec]-700 transition-colors line-clamp-1">{prevEntry.title}</span>
            </Link>
          ) : <div className="flex-1 max-w-xs" />}
          {nextEntry ? (
            <Link
              href={`/schedule/plot/detail?id=${nextEntry.id}`}
              className="flex-1 flex flex-col items-end gap-0.5 px-4 py-3 bg-white rounded-xl shadow-sm border border-slate-100 hover:border-[#1fefec]-200 hover:shadow-md transition-all group max-w-xs"
            >
              <span className="text-xs text-slate-400 group-hover:text-[#1fefec]-500 transition-colors">次へ →</span>
              <span className="text-sm font-medium text-slate-700 group-hover:text-[#1fefec]-700 transition-colors line-clamp-1">{nextEntry.title}</span>
            </Link>
          ) : <div className="flex-1 max-w-xs" />}
        </nav>
      </div>
